...
public class BasicConsumer2 {
    ...
    public static void main(String[] args) {
        ...
        final KafkaProducer<String, String> producer = new KafkaProducer<>(propsp);
        ...
        while (true) {
            ...
            for (final ConsumerRecord<String, String> record : records) {
                ...
                final ProducerRecord<String, String> r = new ProducerRecord<>(defaultTopic2,record.key(), record.value());
                producer.send(r);
                final Future<RecordMetadata> future = producer.send(r);
                // DI DEFAULT ASPETTO ACK
                try {
                    RecordMetadata ack = future.get();
                    // controllo se è stato inviato a kafka
